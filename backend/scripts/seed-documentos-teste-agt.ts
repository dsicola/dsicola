 ...
 *
 * Uso: npx tsx scripts/seed-documentos-teste-agt.ts [instituicaoId]
...
async function main() {
  ...
  const { hash: h1, hashControl: hc1 } = calcularHashFiscal(ft1Num, hoje, '50000', nif, alunoComNif.id);
  ...
  const { hash: h2, hashControl: hc2 } = calcularHashFiscal(ft2Num, hoje, '25000', nif, alunoComNif.id);
  ...
  const { hash: h6, hashControl: hc6 } = calcularHashFiscal(ft6Num, hoje, String(tot6), nif, alunoComNif.id);
  ...
  const { hash: h7, hashControl: hc7 } = calcularHashFiscal(ft7Num, hoje, tot7.toFixed(2), nif, alunoComNif.id);
  await prisma.documentoFinanceiro.create({
    data: {
      instituicaoId: instId,
      tipoDocumento: 'FT',
  ...
  const { hash: h8, hashControl: hc8 } = calcularHashFiscal(ft8Num, hoje, '100', nif, alunoComNif.id);
  await prisma.documentoFinanceiro.create({
    data: {
      instituicaoId: instId,
      tipoDocumento: 'FT',
  ...
  const { hash: h9, hashControl: hc9 } = calcularHashFiscal(ft9Num, hoje, '35', nif, alunoSemNif.id);
  await prisma.documentoFinanceiro.create({
    data: {
      instituicaoId: instId,
      tipoDocumento: 'FT',
