...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { nome, email, senha, instituicaoNome, portalUrl }: WelcomeEmailRequest = await req.json();

    console.log(`Sending welcome email to secretaria: ${email}`);
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [email],
        subject: `Bem-vindo(a) ao ${instituicaoNome} - Acesso ao Sistema`,
        html: `
          <!DOCTYPE html>
          <html>
          <head>
          ...
          <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background: #f9f9f9; padding: 30px; border: 1px solid #e0e0e0; border-top: none;">
              ...
              <p>Seu cadastro como membro da <strong>Secretaria</strong> foi realizado com sucesso!</p>
              
              <p>Utilize os dados abaixo para acessar o sistema:</p>
              ...
              <div style="background: #fff3cd; border: 1px solid #ffc107; border-radius: 6px; padding: 15px; margin-top: 20px;">
                <p style="margin: 0; color: #856404; font-size: 14px;">
                  <strong>⚠️ Importante:</strong> Recomendamos que você altere sua senha após o primeiro acesso para maior segurança.
                </p>
              </div>
            </div>
