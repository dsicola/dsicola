...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'semestres' AND column_name = 'data_inicio_notas'
  ) THEN
    RAISE NOTICE '✅ semestres.data_inicio_notas: EXISTE';
  ELSE
    RAISE NOTICE '❌ semestres.data_inicio_notas: NÃO EXISTE';
  END IF;
  
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'semestres' AND column_name = 'data_fim_notas'
  ) THEN
    RAISE NOTICE '✅ semestres.data_fim_notas: EXISTE';
  ELSE
    RAISE NOTICE '❌ semestres.data_fim_notas: NÃO EXISTE';
  END IF;
  
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'trimestres' AND column_name = 'data_inicio_notas'
  ) THEN
    RAISE NOTICE '✅ trimestres.data_inicio_notas: EXISTE';
  ELSE
    RAISE NOTICE '❌ trimestres.data_inicio_notas: NÃO EXISTE';
  END IF;
  
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'trimestres' AND column_name = 'data_fim_notas'
  ) THEN
    RAISE NOTICE '✅ trimestres.data_fim_notas: EXISTE';
  ELSE
    RAISE NOTICE '❌ trimestres.data_fim_notas: NÃO EXISTE';
  END IF;
