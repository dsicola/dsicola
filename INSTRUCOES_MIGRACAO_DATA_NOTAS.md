# 🔧 INSTRUÇÕES: Aplicar Migração - data_inicio_notas e data_fim_notas
...
Execute o arquivo `backend/APLICAR_MIGRACAO_DATA_NOTAS.sql` diretamente no banco de dados:

**Via psql:**
```bash
psql -U seu_usuario -d seu_banco -f backend/APLICAR_MIGRACAO_DATA_NOTAS.sql
```

**Via pgAdmin ou DBeaver:**
1. Abra o arquivo `backend/APLICAR_MIGRACAO_DATA_NOTAS.sql`
2. Execute o script completo

**Via cliente PostgreSQL:**
```sql
-- Copie e cole o conteúdo de backend/APLICAR_MIGRACAO_DATA_NOTAS.sql
```
...
- Apenas adiciona as colunas necessárias
