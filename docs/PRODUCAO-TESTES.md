...
| **Avaliações e notas (disciplina) / turma** | Período de lançamento, lançamento de notas, pautas | `cd backend && npm run test:lancamento-notas`; `npm run test:periodo-lancamento-notas`; período multi-tenant: `npx tsx scripts/test-periodo-lancamento-multitenant.ts` | full-system: fluxo notas/avaliações, “Professor: notas”; `npm run test:e2e -- e2e/matricula-notas.spec.ts` |
...
| **Académico (geral)** | Cursos, turmas, matrículas, diferenciação Sec/Sup | `cd backend && npm run test:fluxo-planos-secundario-superior`; `npm run test:matricula-turma-disciplina`; `npm run test:diferenciacao-sec-sup` | full-system: “Gestão Acadêmica”, “Gestão Alunos”, “Plano de Ensino, avaliações/notas, presenças” |
