...
echo "  2. ✅ Validar estado atual do banco"
echo "  3. ✅ Popular tabela professores"
echo "  4. ✅ Migrar plano_ensino.professor_id (users.id → professores.id)"
echo "  5. ✅ Atualizar constraints (foreign keys)"
echo "  6. ✅ Validar resultados"
echo ""
...
# ============================================================
# ETAPA 5: ATUALIZAR CONSTRAINTS
# ============================================================
echo -e "${BLUE}[ETAPA 5/6] Atualizando constraints...${NC}"
if psql "$DATABASE_URL" -f "$SCRIPT_DIR/06_atualizar_constraints.sql" > /tmp/migracao_constraints.log 2>&1; then
  cat /tmp/migracao_constraints.log
  echo -e "${GREEN}✅ Constraints atualizadas${NC}"
else
  echo -e "${RED}❌ ERRO: Falha ao atualizar constraints${NC}"
  echo "Log:"
  cat /tmp/migracao_constraints.log
  exit 1
fi
...
# ============================================================
# ETAPA 6: VERIFICAÇÃO PÓS-MIGRAÇÃO
# ============================================================
echo -e "${BLUE}[ETAPA 6/6] Verificando resultados...${NC}"
